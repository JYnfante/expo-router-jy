...
function findTopStateForTarget(state: ResultState) {
	...
	const newCurrent = current?.routes?.[current?.routes?.length - 1];
...
export function isMovingToSiblingRoute(
	...
	if (!currentState.routeNames?.includes(targetRoute?.name as string)) {
		return false;
	}
...
export function getQualifiedStateForTopOfTargetState(
	...
	if (newCurrent != undefined) {
		while (newCurrent.state != null) {
			const nextRoute: any = newCurrent;

			const nextCurrentRoot: InitialState | undefined =
				currentRoot?.routes?.find(
