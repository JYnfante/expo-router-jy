...
function matchAgainstConfigs(
	...
	for (const config of configs) {
		...
		remainingPath = remainingPath.replace(match[1] as string, '');
...
function equalHeritage(a: string[], b: string[]): boolean {
	...
	for (let i = 0; i < a.length; i++) {
		if (a[i]?.localeCompare(b[i] as string) !== 0) {
			return false;
		}
	}
	return true;
...
const createNormalizedConfigs = (
	...
	if (typeof config === 'string') {
		...
		configs.push(
			createConfigItem(screen, routeNames, pattern, config, false)
		);
...
const createNestedStateObject = (
	...
	let initialRoute = findInitialRoute(
		route.name,
		parentScreens,
		initialRoutes
	);
	...
	if (routes.length > 0) {
		...
		while ((route = routes.shift() as ParsedRoute)) {
			...
			const newState = nestedState.routes[nestedStateIndex];

			if (newState != undefined) {
				newState.state = createStateObject(
					initialRoute,
					route,
					routes.length === 0
				) as InitialState;
			...
			if (routes.length > 0 && newState != undefined) {
				nestedState = newState.state as InitialState;
			}
...
const parseQueryParams = (
	...
	const params = queryString.parse(query as string);
