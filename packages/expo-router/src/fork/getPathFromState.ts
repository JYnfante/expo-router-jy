...
function walkConfigItems(
	...
	while (route.name in configs) {
		...
		if (inputPattern == null) {
			...
			throw new Error(
				'Unexpected: No pattern found for route ' + route.name
			);
		}
		pattern = inputPattern;

		if (route.params) {
			const params = processParamsWithUserSettings(
	...
	if (pattern == null) {
		throw new Error(
			`No pattern found for route "${
				route.name
			}". Options are: ${Object.keys(configs).join(', ')}.`
		);
	}
...
function getPathFromResolvedState(
	...
	while (current) {
		...
		if (
			...
			configs[
				nextRoute.state.routes?.[nextRoute.state?.index ?? 0]
					?.name as string
			]
		) {
